...
export const browserCrypto = {
  ...
  async generateKeyPairNoPassword(): Promise<{ publicKey: string; privateKey: string }> {
  ...
  async generateKeyPair(password: string, salt: string): Promise<{ publicKey: string; privateKey: string }> {
    ...
    const encryptedPrivateKey = await this.encryptPrivateKeyWithPassword(privateKeyPem, password, salt)

    return { publicKey, privateKey: encryptedPrivateKey }
  },

  async encryptPrivateKeyWithPassword(privateKey: string, password: string, salt: string): Promise<string> {
    const encoder = new TextEncoder()
    const keyMaterial = await crypto.subtle.importKey('raw', encoder.encode(password), 'PBKDF2', false, ['deriveKey'])

    const key = await crypto.subtle.deriveKey(
      { name: 'PBKDF2', salt: encoder.encode(salt + '_priv'), iterations: 100000, hash: 'SHA-256' },
      keyMaterial,
  ...
  async decryptPrivateKeyWithPassword(encryptedPrivateKey: string, password: string, salt: string): Promise<string> {
    const encoder = new TextEncoder()
  ...
  async importPrivateKey(pem: string): Promise<CryptoKey> {
    ...
    return crypto.subtle.importKey('pkcs8', binaryDer, { name: 'RSA-OAEP', hash: 'SHA-256' }, true, ['decrypt'])
  },

  async decrypt(encryptedBase64: string, privateKeyPem: string): Promise<string> {
    try {
      ...
      const encryptedKey = combined.slice(offset, offset + keyLength)
      offset += keyLength

      const iv = combined.slice(offset, offset + 16)
      offset += 16
      ...
      const aesKey = await crypto.subtle.decrypt({ name: 'RSA-OAEP' }, privateKey, encryptedKey)
      const key = await crypto.subtle.importKey('raw', aesKey, { name: 'AES-GCM', length: 256 }, false, ['decrypt'])
  ...
  async encryptWithPublicKey(text: string, publicKeyPem: string): Promise<string> {
    ...
    const pemContents = publicKeyPem
      ...
      ?.split(pemFooter)[0]
      .replace(/\s+/g, '')
    if (!pemContents) throw new Error('Invalid public key')
    ...
    // Generate AES key and 16-byte IV
    ...
    const enc = await crypto.subtle.encrypt({ name: 'AES-GCM', iv }, aesKey, encoder.encode(text))
    ...
    const encryptedKeyBuf = await crypto.subtle.encrypt({ name: 'RSA-OAEP' }, publicKey, aesKeyRaw)
    const encryptedKey = new Uint8Array(encryptedKeyBuf)

    // Combine: [2 bytes key length][encryptedKey][iv(16)][authTag(16)][ciphertext]
    const keyLen = encryptedKey.byteLength
    const totalLen = 2 + keyLen + 16 + 16 + encryptedMessage.byteLength
